...
import { Github, LinkedinIcon, Mail, Download, ExternalLink, BarChart3, Database, BrainCircuit, LineChart, Activity, Sparkles } from "lucide-react";
    ...
    <main className="min-h-screen bg-slate-950 text-white overflow-hidden">
          ...
          <motion.div initial={{ opacity: 0, y: 35 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .8 }}>
            ...
            <div className="flex flex-wrap gap-4 mt-8">
              <button className="rounded-2xl px-6 py-4 bg-cyan-400 text-slate-950 hover:bg-cyan-300 shadow-[0_0_35px_rgba(34,211,238,.45)] font-semibold">
                <Download className="inline mr-2 h-4 w-4" /> Download Resume
              </button>
      ...
      <section id="skills" className="relative max-w-7xl mx-auto px-6 py-20">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {skills.map((s) => (
            <div key={s.title} className="rounded-3xl border border-white/10 bg-white/10 backdrop-blur-xl hover:bg-white/[.14] transition p-6">
      ...
      <section id="projects" className="relative max-w-7xl mx-auto px-6 py-20">
        ...
        <div className="grid lg:grid-cols-2 gap-7">
          {projects.map((p) => (
            <div key={p.title} className="group rounded-[2rem] border border-white/10 bg-white/10 backdrop-blur-2xl overflow-hidden hover:border-cyan-300/40 transition">
      ...
      <section id="contact" className="relative max-w-7xl mx-auto px-6 py-20">
        <div className="rounded-[2rem] border border-white/10 bg-white/10 backdrop-blur-2xl p-8 md:p-12 text-center">
          ...
          <div className="flex justify-center gap-4 flex-wrap">
            <button className="rounded-2xl bg-cyan-400 text-slate-950 hover:bg-cyan-300 px-5 py-3 font-semibold"><Mail className="inline mr-2 h-4 w-4" /> Contact Me</button>
            <button className="rounded-2xl border border-white/20 bg-white/5 hover:bg-white/10 px-5 py-3 font-semibold"><Github className="inline mr-2 h-4 w-4" /> GitHub</button>
          </div>
        </div>
      </section>
    </main>
